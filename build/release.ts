...
(() => {
	...
	otherFiles = otherFiles.concat(
		readFiles('node_modules/novel-editor-core/min-maps/**/*', {
			base: 'node_modules/novel-editor-core/'
		})
	);
	otherFiles = otherFiles.concat(
		readFiles('node_modules/novel-editor-core/LICENSE', {
			base: 'node_modules/novel-editor-core/'
		})
	);
...
function AMD_releaseOne(type: 'dev' | 'min') {
	const coreFiles = readFiles(`node_modules/novel-editor-core/${type}/**/*`, {
		base: `node_modules/novel-editor-core/${type}`
	});
	...
	for (const file of files) {
		...
		const extraContent = pluginFiles.map((file) => {
			return file.contents
				.toString()
				.replace(
					/define\((['"][a-z\/\-]+\/fillers\/novel-editor-core['"]),\[\],/,
...
function ESM_release() {
	const coreFiles = readFiles(`node_modules/novel-editor-core/esm/**/*`, {
		base: 'node_modules/novel-editor-core/esm',
		// we will create our own editor.api.d.ts which also contains the plugins API
		ignore: ['node_modules/novel-editor-core/esm/vs/editor/editor.api.d.ts']
	});
 ...
 * Rewrites imports for 'novel-editor-core/**'
 */
function ESM_releasePlugins() {
	...
	for (const file of files) {
		...
		for (let i = info.importedFiles.length - 1; i >= 0; i--) {
			...
			if (!/(^\.\/)|(^\.\.\/)/.test(importText)) {
				// non-relative import
				if (!/^novel-editor-core/.test(importText)) {
					console.error(`Non-relative import for unknown module: ${importText} in ${file.path}`);
					process.exit(1);
				}

				if (importText === 'novel-editor-core') {
					importText = 'novel-editor-core/esm/vs/editor/editor.api';
				}

				const importFilePath = importText.substring('novel-editor-core/esm/'.length);
 ...
 */
function releaseDTS() {
	const monacodts = readFiles('node_modules/novel-editor-core/monaco.d.ts', {
		base: 'node_modules/novel-editor-core'
	})[0];
...
function releaseThirdPartyNotices() {
	const tpn = readFiles('node_modules/novel-editor-core/ThirdPartyNotices.txt', {
		base: 'node_modules/novel-editor-core'
	})[0];
